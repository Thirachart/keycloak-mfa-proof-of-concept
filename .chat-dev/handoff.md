...
- Verify Email now uses the custom `poc-main` email theme based on local `template/verify-email.html`. The Thai PFAS HTML/text templates are active at runtime, `template/newlogo.png` is embedded as an exact Base64 data URI, and the realm sets `emailTheme=poc-main`. A real Mailpit message was validated for exact Thai subject/body, Base64 logo, action-token URL and orange verify button; Chrome rendered the Base64 PNG successfully (`naturalWidth=238`), followed the verification action, and a temporary test user changed to `emailVerified=true` before cleanup.
- Verify/Update Email browser pages now share the PFAS theme. `UPDATE_EMAIL` is registered as an AIA with `verifyEmail=true`, oauth2-proxy allows both `VERIFY_EMAIL` and `UPDATE_EMAIL`, and the Verify Email pages expose `เปลี่ยนอีเมล`. The update form, `Confirmation email sent`, and `Email updated` screens were exercised in Chrome with PFAS background/card/logo/button styling. The update confirmation email also uses the PFAS HTML/text template with Base64 logo. End-to-end test changed demo to a generated email, confirmed through the real Mailpit action-token, verified both status screens, then restored demo to its pre-test email and restored the disabled no-email test user baseline.
