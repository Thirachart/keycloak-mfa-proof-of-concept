...
- `keycloak/themes/poc-main/login/resources/img/login-img.png` — local copy of the production PFAS left-panel artwork referenced by `template/configmap-pattaya-theme.yaml`; the same full-bleed artwork is used by Login, MFA selector, Email OTP, password update/reset, and inherited PATTAYA browser pages for consistent production visual identity
...
- Added `keycloak/themes/poc-main/login/template.ftl` as the shared PATTAYA shell for reachable Keycloak pages that previously inherited the stock `keycloak.v2` shell.
- Kept `login.ftl`, `login-update-password.ftl`, `mfa-method-selector.ftl`, `email-code-form.ftl`, and `error.ftl` on their existing production-derived/custom PATTAYA implementations without changing authentication behavior. Follow-up presentation fixes normalized inherited form controls, made Thai the default user-facing message language, removed duplicate OTP validation alerts, and made the Thai Error-page heading visually primary.
- Kept `verify-layout.ftl`, `login-verify-email.ftl`, `update-email.ftl`, and the special Verify/Update branches in `info.ftl` structurally unchanged; only `poc.css` visual tokens were aligned with the PATTAYA theme.
...
| Successful Reset Password ended on stock `Account updated` with no clear next action | Keycloak's generic password-update success uses `accountUpdatedTitle` / `accountPasswordUpdatedMessage` and the generic `info.ftl` may have no useful return link in the recovery context | added a recovery-only password-success branch in `info.ftl` (password updated with no client redirect/action) with Thai PFAS copy and a primary `เข้าสู่ระบบด้วยรหัสผ่านใหม่` action; authenticated AIA flows with a client redirect continue back to the application; added Thai fallback translations for generic account-updated keys | static/theme validation confirms the dedicated recovery branch and login action are packaged; browser recovery-flow spot-check remains manual |
